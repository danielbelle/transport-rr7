...
import type { PdfLiveProps } from "~/components/types";
import { fieldConfig } from "~/components/ui/fieldConfig";

export default function PdfLive({ formData, onPdfGenerated }: PdfLiveProps) {
  ...
  const loadPdfTemplate = useCallback(async (): Promise<ArrayBuffer> => {
    ...
    try {
      ...
      throw error; // Propaga o erro em vez de criar fallback
    } finally {
